...
\begin{prooftree}
    ...
    \noLine
    \BinaryInfC{$\Gamma, x:A', \vec{a} : \PCtx, k_1: B_1 \to (\PCtx,\vec{b}_1)^{|\vec{b}_1|}\to \cC\pbr{\vec{b}_1}, \dots, k_p: B_p \to (\PCtx,\vec{b}_p)^{|\vec{b}_p|}\to \cC\pbr{\vec{b}_p} \vdash c_{\op} : \cC$}
    \UnaryInfC{$\Gamma \vdash (\op[\vec{a}](x; k_1, \dots , k_p) \mapsto c_{\op}); H : A!\Sigma \cup \{op\} \Rightarrow \cC$}
\end{prooftree}
...
\begin{prooftree}
    ...
    \noLine
    \TrinaryInfC{$\Gamma,y_1:B_1, \vec{b}_1: \PCtx \vdash c_1 : A!\Sigma \quad \cdots \quad \Gamma,y_p:B_p, \vec{b}_p: \PCtx \vdash c_p : A!\Sigma$}
    \UnaryInfC{$\Gamma\vdash \op[\vec{a}](v; [\vec{b}_1]y_1.c_1, \dots, [\vec{b}_p]y_p.c_p): A!\Sigma$}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{}
    \UnaryInfC{\lstinline{do x <- op(v; [vecb1]y1.c1,...,[vecbp]yp.cp) in c leadsto op(v; [vecb1]y1.do x <- c1 in c,...,[vecbp]yp.do x <- cp in c)}}
\end{prooftree}

For \lstinline|h = handler { return x mapsto c_r, ..., op[vecq](v; k_1,..., kn) mapsto c_op, ...}|, we have:
...
\begin{prooftree}
    \AxiomC{}
    \UnaryInfC{\lstinline{with h handle op[veca](v; [vecb1]y1.c1,..., [vecbp]yp.cp) leadsto c_op[v/x, veca/vecq, lambdavecb1y.with h handle c1/k1,..., lambdavecbpyp.with h handle cp/kp]}}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{}
    \UnaryInfC{\lstinline{with h handle op[veca](v; [vecb1]y1.c1,..., [vecbp]yp.cp) leadsto op[veca](v; [vecb1]y1.with h handle c1,..., [vecbp]yp.with h handle cp)}}
\end{prooftree}
...
\begin{prooftree}
    ...
    \AxiomC{$\Gamma\vdash\op[\vec{a}](v; [\vec{b}_1]y_1.c_1,\dots,[\vec{b}_p]y_p.c_p) : B!\Sigma'$}
    \BinaryInfC{$\Gamma \vdash$ \lstinline{with h handle op(v; [vecb1]y1.c1,..., [vecbp]yp.cp)} $: A!\Sigma$}
\end{prooftree}
Given the structure of $\op\in h$, the handler typing derivation leads to for some $h' \subset h$ and $\op\notin h'$:
\begin{prooftree}
    \AxiomC{$\op : (|\vec{q}|, A') \hookrightarrow (|\vec{r}_1|, B_1; \dots; |\vec{r}_p|,B_p)$}
    \AxiomC{$\Gamma \vdash h' : B!\Sigma' \Rightarrow A!\Sigma$}
    \noLine
    \BinaryInfC{$\Gamma, x:A', \vec{q} : \PCtx, k_1: B_1 \to (\PCtx,\vec{r}_1)^{|\vec{r}_1|}\to (A!\Sigma)\pbr{\vec{r}_1}, \dots, k_p: B_p \to (\PCtx,\vec{r}_p)^{|\vec{r}_p|} \to (A!\Sigma)\pbr{\vec{r}_p} \vdash c_{\op} : A!\Sigma$}
    \UnaryInfC{$\vdots$}
...
\begin{prooftree}
    ...
    \UnaryInfC{$\Gamma\vdash\op[\vec{a}](v; [\vec{b}_1]y_1.c_1,\dots,[\vec{b}_p]y_p.c_p) : B!\Sigma'$}
\end{prooftree}
...
\begin{prooftree}
    \AxiomC{$\vdots$}
    \UnaryInfC{$\Gamma \vdash$ \lstinline|c_op[v/x, veca/vecq, lambdavecb1y.with h handle c1/k1,..., lambdavecbpyp.with h handle cp/kp]| $:A!\Sigma$}
\end{prooftree}
as required.
\end{proof}
