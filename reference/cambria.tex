...
\lstdefinelanguage{cambria}{
  morekeywords={fun,rec,let,and,infixl,infixr,infix,handler,return,finally,do,
                in,if,then,else,with,handle,inl,inr,case,of,effect,Unit,Int,
                Bool,Double,Str,Name,Map,List,true,false},
  sensitive=true,
  morecomment=[l]{--},
  morestring=[b]",
  literate=
...
Types are written in \cba{effect} statements using this syntax:
...
Note that \texttt{;} sequences two complete computations, so a \texttt{;}
placed after the body of a \cba{do} closes the whole \cba{do} chain: in
\cba{do x <- c in c1 ; c2}, the variable \texttt{x} is not in scope in
\texttt{c2}.  Use another \cba{do} (or \cba{let}) when the binding must
scope over the continuation.
...
\begin{lstlisting}
infixr 1 => = !mkImpl.    -- effect operation as infix
infixl 3 <+ = snoc.       -- ordinary function as infix
\end{lstlisting}

\cba{infixl}, \cba{infixr} and \cba{infix} declare a left-, right- and
non-associative operator respectively, at the given precedence level (1 to
9, loosest to tightest, on the same scale as the built-in table above:
\texttt{::} sits at 2, \texttt{==} at 3, \texttt{++} at 4, \texttt{+} at 5,
\texttt{*} at 6).  The right-hand side is either an operation name
\texttt{!op} or a function name; \texttt{a OP b} then elaborates to
\texttt{!op (a, b)} or \texttt{f (a, b)}, with operands evaluated left to
right.
...
The reserved-but-unused tokens \texttt{=>}, \texttt{\&\&}, \texttt{||},
\texttt{<}, \texttt{>}, \texttt{<=}, \texttt{>=}, \texttt{/=} may also be
declared.  Built-in operators cannot be redeclared, using an undeclared
operator is an error, and non-associative operators cannot be chained.
...
\texttt{[\$p -> Type]} list placed between the handler and \cba{handle}:
...
\begin{lstlisting}
with handler { ... } [$p -> Name, $q -> Int] handle (...)
\end{lstlisting}
...
inner handler supplies the fresh names used as locations.  The program
allocates two references (holding 2 and 3), copies \texttt{b}'s value into
\texttt{a}, and reads \texttt{a} back.  Result: \texttt{Pure: 3}.
...
with handler {
  ...
  -- (2) map at ($loc -> Int): read all refs in the list
  do vals <- map (fun r -> !get r) refs in

  -- (3) map at (Int -> Bool): test each value against 20
...
The same polymorphic \texttt{map} is used at three different type
instantiations: \texttt{Int -> \$loc} (allocating a reference per element),
\texttt{\$loc -> Int} (reading each reference), and \texttt{Int -> Bool}
(testing each value).  \texttt{map} never inspects what \texttt{\$loc} is.
Result: \texttt{Pure: [False, True, False]}.
...
of booleans, e.g.\ \texttt{Pure: (False, True)}.
...
state from above, with its fresh-name handler underneath:
...
\begin{itemize}
  ...
  \item It instantiates \texttt{\$urn -> \$loc}: the urn parameter becomes a
        location of the \emph{outer} local-state handler, itself an abstract
        parameter.
  \item The \textbf{middle handler} provides the function-encoded local state
        (\texttt{\$loc -> \$name}), and the \textbf{outer handler} supplies the
        fresh location names (\texttt{\$name -> Int}).
...
comp    := comp ; comp
         ...
         | case expr of { inl p -> comp,
                          inr p -> comp }
         | with expr paramInsts handle comp
...
-- Parameter instantiations (optional, between handler and handle)
