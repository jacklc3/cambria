...
\begin{itemize}
  ...
  \item Literal patterns: integers, booleans, and strings
\end{itemize}
...
9 are left free for fixity declarations
...
\cba{with ... handle}, and the scope of an \cba{effect} or fixity declaration
all extend as far to the right as they can, across \texttt{;} and to the end
of the enclosing computation.  So
...
\subsection{User-Defined Operators (Fixity Declarations)}
\label{sec:fixity}

A fixity declaration introduces an infix operator and says what it elaborates
to; like \cba{effect} declarations, it is terminated by a dot and scopes over
the rest of the program:

\begin{lstlisting}
infixr 0 => = !mkImpl.    -- effect operation as infix
infixl 8 <+ = snoc.       -- ordinary function as infix
\end{lstlisting}

\cba{infixl}, \cba{infixr} and \cba{infix} declare a left-, right- and
...
unoccupied, so a declaration there is either looser or tighter than
everything built in; the remaining levels tie with the built-in group listed
against them.  The right-hand side is either an operation name \texttt{!op}
or a function name; \texttt{a OP b} then elaborates to \texttt{!op (a, b)} or
\texttt{f (a, b)}, with operands evaluated left to right.

Operator names are sequences of the symbol characters
\verb!+ - * / < > = & | ^ @ % ? ~!, excluding the reserved symbols
(\texttt{->}, \texttt{<-}, \texttt{=}, \texttt{==}, \texttt{::}, \ldots).
\texttt{=>} is reserved for handler types but unused as an operator, so it
too may be declared.  Built-in operators cannot be redeclared, using an
undeclared operator is an error, and non-associative operators cannot be
chained.

For example, with the \texttt{=>} declaration above, Peirce's law is written
...
which elaborates to
\cba{!mkImpl (!mkImpl (!mkImpl (p, q), p), p)}.
...
\cba{fun} replaced by \cba{rec f} under \cba{let rec}.
...
operators into a flat chain and the desugarer reassociates it by precedence
climbing against the fixity table, which is why a fixity declaration can
introduce a new operator without changing the grammar.  A declared operator
elaborates to an application of its target, \texttt{f (x, y)} for a function
and \cba{!op (x, y)} for an operation.
