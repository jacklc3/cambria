...
\lstdefinelanguage{cambria}{
  morekeywords={fun,rec,let,and,infixl,infixr,infix,handler,return,finally,do,
                in,if,then,else,with,handle,inl,inr,case,of,effect,Unit,Void,
                Int,Bool,Double,Str,Name,Map,List,true,false},
  sensitive=true,
  morecomment=[l]{--},
  morestring=[b]",
  literate=
...
\cba{let} binds the result of an expression or computation to a pattern:
...
\cba{let x = e in c} is shorthand for \cba{do x <- return e in c} (or
\cba{do x <- e in c} when \texttt{e} is a computation).

With argument patterns, \cba{let} defines a function, and \cba{let rec} a
recursive function:
...
These are shorthand for binding a \cba{fun} or \cba{rec} value:
\cba{let f x = c in ...} desugars to \cba{do f <- return (fun x -> c) in ...}.

Mutually recursive functions are joined with \cba{and}:

\begin{lstlisting}
let rec even n = if n == 0 then return true  else odd (n - 1)
and     odd  n = if n == 0 then return false else even (n - 1)
in even 10
\end{lstlisting}

This desugars to a single recursive function over a tagged sum (one summand
per definition), together with a wrapper binding for each name; the
definitions must therefore agree on their result type.

A \cba{let} may also bind an infix operator by carrying a fixity, which is
the only way to introduce one (Section~\ref{sec:fixity}):
...
\cba{let} bindings, handler clauses, and \cba{case} branches.

Binding positions (\cba{do}, \cba{fun}, \cba{rec}, \cba{let}, handler
clauses) accept only \emph{irrefutable} patterns, which always match:

\begin{itemize}
  \item A variable name (e.g.\ \texttt{x}): binds the matched value
  \item A wildcard \cba{_}: matches anything, discards the value
  \item The unit pattern \cba{()}
  \item A pair pattern \texttt{(\emph{p1}, \emph{p2})}: destructs a pair,
        matching each component with a sub-pattern
\end{itemize}

\cba{case} branches additionally accept \emph{refutable} patterns, which can
fail to match (Section~\ref{sec:control}):

\begin{itemize}
  \item Sum patterns \cba{inl} \emph{p} and \cba{inr} \emph{p}
  \item List patterns \cba{[]} and \emph{p1} \cba{::} \emph{p2}
  \item Literal patterns: integers, doubles, booleans, and strings.  Numeric
        literals may be signed, so \cba{-1} and \cba{-0.25} are patterns; a
        literal pattern is tested with \texttt{==}
\end{itemize}

All of these nest freely, e.g.\ \cba{inl (x :: (1, _) :: rest)}.
...
Pair patterns desugar into pair projections
...
\cba{case} matches a value against a sequence of clauses:
...
The scrutinee may be a computation; it is evaluated once, before matching.
Clauses are tried top to bottom and the first matching clause wins, so an
earlier clause may overlap a later one:
...
Matching compiles down to the kernel's binary sum \cba{case}, list
destructuring and equality tests (Section~\ref{sec:desugaring}); a two-clause
\cba{inl}/\cba{inr} match maps directly onto the kernel construct.  If no clause matches at runtime the
program aborts with a pattern-match failure, so end with an irrefutable
clause when the earlier clauses are not exhaustive.
...
Every infix operator sits at one of ten numbered levels, 0 (loosest) to 9
(tightest).  The built-in operators occupy seven of them, and levels 0, 8 and
9 are left free for operator bindings
(Section~\ref{sec:fixity}):
...
0 & \emph{(free)}   & \\
1 & \texttt{::} (cons) & right \\
2 & \texttt{||} (disjunction) & right \\
3 & \texttt{\&\&} (conjunction) & right \\
4 & \texttt{==} \texttt{/=} \texttt{<} \texttt{>} \texttt{<=} \texttt{>=}
    (comparison) & none \\
5 & \texttt{++} (string concat) & left \\
6 & \texttt{+} \texttt{-} (additive) & left \\
7 & \texttt{*} \texttt{/} (multiplicative) & left \\
8, 9 & \emph{(free)} & \\
\bottomrule
\end{tabular}
\end{center}

Sequencing with \texttt{;} is looser than every level, and function
application binds tighter than every level.

All infix operators desugar into function applications.  For example,
\texttt{x + y} becomes \texttt{(+) (x, y)}.

The compound computation forms are looser still.  The body of a \cba{do} or a
\cba{let}, the branches of an \cba{if} and a \cba{case}, the body of a
\cba{with ... handle}, and the scope of an \cba{effect} declaration all extend
as far to the right as they can, across \texttt{;} and to the end of the
enclosing computation.  So
...
An operator is not a separate kind of thing: it is an ordinary variable whose
name happens to be symbolic, and the built-in operators are exactly that
already (\texttt{+} names the primitive that adds a pair).  So an operator is
introduced by a \cba{let} carrying a fixity, and the two spellings below bind
the same thing:
...
The first names the operands and takes the body directly.  The second gives
the operator a function, which must accept a \emph{pair}, since \cba{a OP b}
always elaborates to \cba{(OP) (a, b)} with the operands evaluated left to
right.  Writing a curried function there is a type error rather than a
special case.  Both forms are available under \cba{let rec}, where the
operator is in scope in its own body, and inside an \cba{and} group, where
every operator in the group is in scope in every body.

\cba{infixl}, \cba{infixr} and \cba{infix} give a left-, right- and
non-associative operator respectively, at the given precedence level, on the
same 0 to 9 scale as the built-in operators above.  Levels 0, 8 and 9 are
unoccupied, so a binding there is either looser or tighter than everything
built in; the remaining levels tie with the built-in group listed against
them.

Because the fixity belongs to the binding, it has exactly the binding's
scope.  An operator may shadow a built-in one, and both its meaning and its
associativity revert at the end of the enclosing computation:
...
Operator names are sequences of the symbol characters
\verb!+ - * / < > = & | ^ @ % ? ~!.  Exactly four spellings are unavailable,
because the grammar needs them elsewhere: \texttt{->}, \texttt{<-},
\texttt{\textasciitilde>} and \texttt{=}.  Everything else in that alphabet is
free, including every built-in operator and \texttt{=>}, which a binding
shadows in the ordinary way.  Note that \texttt{:} is not in the alphabet, so
\texttt{::} and type ascription stay unambiguous.

Using an operator with no fixity in scope is an error, and non-associative
operators cannot be chained.

Parenthesising an operator gives back the plain variable, so operators can be
applied prefix and passed as arguments:
...
With the \texttt{=>} binding above, Peirce's law is written
...
which elaborates to
\cba{(=>) ((=>) ((=>) (p, q), p), p)}, and so to three calls of
\cba{!mkImpl}.
...
with \cba{return e} dropped when \texttt{e} is already a computation, and
\cba{fun} replaced by \cba{rec f} under \cba{let rec}.  An operator binding is
the same \cba{do}, binding the symbol as a variable that takes a pair; the
fixity is consumed while desugaring and never reaches the core.

\subsection{Mutual Recursion}

\begin{lstlisting}
let rec f x = c1 and g y = c2 in body
\end{lstlisting}

desugars to a single recursive function over a tagged sum, one summand per
definition, plus a wrapper binding for each name that injects its arguments
and calls it (Bekic's construction):

\begin{lstlisting}
do h <- return (rec h a -> case a of {
     inl x -> c1,
     inr y -> c2
   }) in
do f <- return (fun x -> h (inl x)) in
do g <- return (fun y -> h (inr y)) in
body
\end{lstlisting}

The wrappers are also in scope inside the body of \texttt{h}, which is what
lets \texttt{c1} call \texttt{g} and \texttt{c2} call \texttt{f}.  Because the
definitions share one function, they must agree on their result type.

\subsection{Multi-Clause \texttt{case}}

A two-clause \cba{inl}/\cba{inr} match on irrefutable sub-patterns is the
kernel \cba{case} construct directly.  Anything else compiles to a decision
tree: each clause becomes a test that either enters the body or calls a
fall-through thunk holding the remaining clauses.
...
\begin{lstlisting}
do _rest <- return (fun _ -> {- clause 2 -}) in
...
List patterns go through \cba{_uncons}, pair patterns through \cba{_fst} and
\cba{_snd}, and literal patterns through \cba{_eq} followed by an \cba{if}.
When every clause fails the tree ends in \cba{_matchfail}, which aborts.

These are internal aliases for the corresponding primitives, with
\cba{_matchfail} existing only for this purpose.  Desugaring goes through them
rather than through \cba{uncons}, \cba{fst}, \cba{snd} and \cba{==} so that
shadowing a primitive cannot change what a pattern means.  A leading
underscore is not a legal identifier in the surface, so no program can bind or
even mention one, which is also why the generated \cba{_rest} and \cba{_tmp}
above cannot collide with a variable of the user's.
...
Declared operators are no different: \texttt{x <+ y} becomes
\texttt{(<+) (x, y)}, a reference to the variable the binding introduced.

The parser does not resolve precedence itself.  It collects a run of infix
operators into a flat chain, and the desugarer reassociates the chain by
precedence climbing against the fixities in scope, which is why a new
operator needs no change to the grammar.
