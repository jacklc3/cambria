...
\lstdefinelanguage{cambria}{
  morekeywords={fun,rec,handler,return,finally,do,in,if,then,else,with,handle,
                inl,inr,case,of,effect,Unit,Int,Bool,Double,Str,Unique,Map,List,
                true,false},
  sensitive=true,
  morecomment=[l]{--},
  morestring=[b]",
...
\texttt{Unique} & (produced by \cba{!fresh}) & Opaque unique identifiers \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
\texttt{!fresh ()}    & \texttt{Unit $\leadsto$ Unique}    & Fresh unique identifier \\
