...
Matching compiles down to the kernel's binary sum \cba{case}, the built-in
\cba{uncons}, and equality tests; a two-clause \cba{inl}/\cba{inr} match maps
directly onto the kernel construct.  If no clause matches at runtime the
...
List patterns go through the built-in \cba{uncons}, pair patterns through
\cba{fst} and \cba{snd}, and literal patterns through \cba{==} followed by an
\cba{if}.  When every clause fails the tree ends in the built-in
\cba{matchfail}, which aborts.
