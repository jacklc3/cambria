...
\text{Effect signatures}       & \Sig   & ::= & \{ \op_i : \arity{A_i}{B_i} \}_{i=1}^{m} \\[4pt]
  ...
  \text{Parameter context}     & \Psi   & ::= & \emptyset \mid \Psi, \psi \\[4pt]
...
That are defined inductively over the language syntax.
\begin{mathpar}
  \inferrule
...
\begin{array}{lcll}
  ...
  \with h, \sigma \handle \ret v
    ...
    & \leadsto & c_i\bigl[v/x,\;(\fun y \mapsto \with h, \emptyset \handle c)/k\bigr]
    & (\beta\text{-op}) \\
...
\begin{align*}
  \with v, \{\psi \mapsto A\} \handle c \quad = &\quad \unpack \pack(\psi_n, A_n, x_{n-1}) \as (\psi_n, x_n) \iin (\with x_n \handle c).
\end{align*}
...
We update the handle syntax and extend the value types, values and computations with the following constructs.
\[
\begin{array}{lrcl}
...
The operational semantics replaces parametrized handler reduction with classic reduction and adds a new reduction for the existential constructs.
\[
\begin{array}{lcll}
  \with h \handle \ret v
    & \leadsto & c_r[v/x]
...
rather than having to revert to inducting on the size of the computation tree. While we only indtroduce the relation on the core syntax,
...
The ease of integration primarily comes from the decomosition of parametrized handlers into plain handlers and existentials, making each induction case canonical.
...
The logical relation $\llbracket {-} \rrbracket$ on values, schemes, computations, and environments is given by induction.
\begin{align*}
  ...
  \VV\llbracket \forall\vec\alpha.A \rrbracket_\delta &= \{(v_1, v_2)_n \mid \exists R \in \Rel.\; (v_1, v_2)_n \in \VV\llbracket A \rrbracket_{\delta[\psi \mapsto R]}\} \\
  \SSS\llbracket \exists\psi.A \rrbracket_\delta &= \{(v_1, v_2)_n \mid \forall \vec{R} \in \Rel^{|\vec\alpha|}.\; (v_1, v_2)_n \in \VV\llbracket A \rrbracket_{\delta[\vec\alpha \mapsto \vec{R}]}\} \\
  \CC\llbracket \comp{A}{\Sig} \rrbracket_\delta &= \{(c_1, c_2)_n \mid c_1 \text{ and } c_2 \text{ are terminally related at } n\} \\
...
\begin{definition}
  Two computations $c_1$ and $c_2$ are \emph{terminally related} at $n$ if for all $j < n$:
  \begin{itemize}
  ...
  \end{itemize}
\end{definition}
The step index $j$ counts reduction steps on the left.
The self-reference of $\CC\llbracket \comp{A}{\Sig} \rrbracket_\delta$ is well-founded because $k < n{-}j$ ensures $k < n$.
...
\emph{Handler:} inner induction on $j$. At $j = m{+}1$, the return case applies the fundamental theorem to $c_r$; the operation case constructs the deep continuation $\fun y \mapsto \with h \handle c'$, which is related at $\VV_{m+1}\llbracket \tfun{B_\op}{\comp{B}{\Sig'}} \rrbracket$ by the inner induction hypothesis at $j' \leq m$.
...
This section demonstrated that \cambria{} elaborates faithfully into the core language.
This means that the parametricity result from \Cref{thm:parametricity} carries into \cambria{}, and the pbstract interfaces of the parametrized handlers are formally guaranteed.

\begin{definition}
  The \emph{elaboration translation} $\elab{{-}}$ from \cambria{}'s syntax to the core syntax is given the identity on types,
  and structural identity on terms, apart from the following clause, which is defined inductively over the size of the parameter substitution.
  \begin{align*}
    \elab{\with v, \emptyset \handle c}              &= \with v \handle c \\[-2pt]
    \elab{\with v, \sigma[\psi \mapsto A] \handle c} &= \unpack \pack(\psi, A, v) \as (\psi, x) \iin \elab{\with x, \sigma \handle c}
  \end{align*}
\end{definition}

The following results confirm that this elaboration is sound.

\begin{proposition}[Type Preservation]
  If $\cjp \Theta \Psi \Gamma c C$ in \cambria{}, then $\cjp \Theta \Psi \Gamma {\elab{c}} C$ in the core syntax.
\end{proposition}

\begin{proof}
  The only non-trivial case is the $\handle$ elaboration. This is proved by induction on the size of the parameter substitution.
  ...
  \]
  When $\sigma=\sigma'[\psi \mapsto A]$, working back from the typing derivation gives:
  \[
    \inferrule{
  ...
  \[
    \cjp \Theta {\Psi, \psi} {\Gamma, x : \thandler{C[\sigma']}{D}} {\with x, \sigma' \handle c} D
  \]
  By the IH, $\cjp \Theta {\Psi, \psi} {\Gamma, x : \thandler{C[\sigma']}{D}} {\elab{\with x, \sigma' \handle c}} D$, resulting in
  \[
...
\begin{proposition}[Operational Correspondence]
  $c \leadsto c'$ in \cambria{} if and only if $\elab{c} \leadsto^* \elab{c'}$ in the core language.
\end{proposition}
