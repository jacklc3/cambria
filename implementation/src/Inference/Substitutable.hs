...
instance Substitutable Type where
  ...
  apply s (TFun t1 t2 e2)   = TFun (apply s t1) (apply s t2) e2
  apply s (THandler t1 e1 t2 e2) = THandler (apply s t1) e1 (apply s t2) e2
  ...
  ftv (TFun t1 t2 _)       = ftv t1 `Set.union` ftv t2
  ftv (THandler t1 _ t2 _) = ftv t1 `Set.union` ftv t2
