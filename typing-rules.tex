...
\begin{mathpar}
  ...
  \inferrule
    {\cj{\G, f : \tfun{A}{\comp{B}{\emptyset}},\, x : A}{c}{B}{\Sig}}
...
\begin{mathpar}
  \inferrule
    {\sigma = \{ p_j \mapsto T_j \}_{j=1}^{m}
    \\\\
    \cj{\G, x_r : A}{c_r}{B}{\Sig'}
    \\\\
    \left[
      (\op_i : A_i \leadsto B_i) \in \Sig \quad
      \cj{\G, x_i : \sigma(A_i),\, k_i : \tfun{\sigma(B_i)}{\comp{B}{\Sig'}}}{c_i}{B}{\Sig'}
    \right]_{i = 1}^{n}
    \\\\
    \Sig \setminus \{\op_1, \ldots, \op_n\} \subseteq \Sig'
    \\
    \ftv_{\mathrm{params}}(\Sig') \cap \mathrm{dom}(\sigma) = \emptyset
    }
    {\vj{\G}{
      \hhandler\;\{\ret x_r \to c_r,\;
        \op_i\; x_i\; k_i \to c_i,\;
        \$p_j \to T_j
      \}
...
The operation clauses are typed with \emph{post-substitution} arities: if $\op_i$ has arity $A_i' \leadsto B_i'$ in~$\Sig$, the clause binds $x_i : \sigma(A_i')$ and $k_i : \sigma(B_i') \to \comp{B}{\Sig'}$.
